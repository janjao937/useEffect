...
const App = () => {
  ...
  const OnSubmitHandeler = async (e) => {
    ...
    let url = `${BaseURL}/search/keyword?query=${Keyword}&page=${currentPage}`;
    let option =
    {
      method: "GET",
      headers: {
  ...
  const OnChangePageHandeler = async ({page }) => {
   
    
    let url = `${BaseURL}/search/keyword?query=${Keyword}&page=${page}`;
    let option =
    {
      method: "GET",
      headers: {
  ...
  return (
    <div className="App">
      ...
      <div>
        <p>{currentPage}</p>
        {Array.from(Array(allPage).keys()).map((n) => (
          <button  onClick={() => OnChangePageHandeler({ page: n + 1 })} key={n + 1}>{n + 1}</button>
        ))}
